...
/**
 * NEW: Generic: list records in a table by where clause (multi-row).
...
export async function insertRecord(table, payload) {
  ...
  for (const version of ["v3", "v2"]) {
    ...
    if (resp.status === 404) {
      // 👇 THIS is the missing visibility you need
...
/**
 * NEW: List all transactions for a given IDKEY (for Weebly transaction history UI).
 */
...
/**
 * NEW: List all transactions related to a given Stripe charge id
