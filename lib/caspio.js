...
export async function getCaspioAccessToken() {
  ...
  if (!resp.ok) {
    // Helpful when Caspio returns HTML or a proxy returns an error page
    const preview = String(text || "").slice(0, 250);
    throw new Error(`Caspio token error ${resp.status}: ${preview}`);
  }
  ...
  try {
    ...
    const preview = String(text || "").slice(0, 250);
    throw new Error(`Caspio token response was not JSON. Body starts: ${preview}`);
  }
