...
//
// Read-only eligibility check for customer cancellation
// - No Caspio token ever exposed to browser
// - Returns { ok:true, eligible:boolean, reason:string, start_time?, window_hours? }

import { getCaspioAccessToken } from "../../lib/caspio"; // <-- align to your project helper
...
export default async function handler(req, res) {
  ...
  try {
    const idkey = String(req.query.idkey || "").trim();
    if (!idkey) return res.status(400).json({ ok: false, error: "Missing idkey" });

    const token = await getCaspioAccessToken();
    if (!token) return res.status(500).json({ ok: false, error: "Server token helper not configured" });
    ...
    if (!billing) return res.status(404).json({ ok: false, error: "Reservation not found" });

    // Reservation status check (already cancelled)
    ...
    const whereRes = `${F_IDKEY}='${escWhereValue(idkey)}' AND ${F_TYPE}='${escWhereValue(TYPE_RESERVATION)}'`;
    const resPath =
    ...
    if (status === "cancelled") {
      return res.status(200).json({ ok: true, eligible: false, reason: "Already cancelled." });
    }

    const chargeType = String(billing?.[VB_CHARGE_TYPE] || "").trim().toLowerCase();
    if (chargeType === "pay now") {
      return res.status(200).json({ ok: true, eligible: false, reason: "Paid in advance (Pay Now) — cannot cancel." });
    }

    const startRaw = billing?.[VB_DATE_START_TIME];
    const startDt = parseDateAny(startRaw);
    if (!startDt) {
      return res.status(200).json({ ok: true, eligible: false, reason: "Missing start time — cannot evaluate cancel window." });
    }
    ...
    if (now.getTime() >= cutoffMs) {
      return res.status(200).json({
        ok: true,
        eligible: false,
        reason: `Within ${winHours} hour cancel window — cannot cancel.`,
        start_time: startDt.toISOString(),
        window_hours: winHours,
      });
    }
    ...
    return res.status(200).json({
      ok: true,
      eligible: true,
      reason: `Eligible (outside ${winHours} hour window).`,
      start_time: startDt.toISOString(),
      window_hours: winHours,
    });
